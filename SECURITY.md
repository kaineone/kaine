...
`kaine/nexus/config.py:12-13`). Every state-changing endpoint and the live
diagnostics stream require the operator token (`kaine/nexus/auth.py`): a
bearer token, or a signed-in session whose unsafe requests also carry the
session key. The token is `[nexus].operator_token` in `config/secrets.toml`
(generated by `python -m kaine.setup`) or `KAINE_NEXUS_TOKEN`, at least 32
characters; with none configured, those endpoints refuse every request. Read
surfaces also require it when the conversation console or
`dev_content_override` is enabled; the default diagnostics page is readable
without it. Nexus refuses a non-loopback bind unless
`[nexus].non_loopback_allowed` is set and a token is configured. The further
defense-in-depth control is the privacy boundary in `kaine/privacy_filter.py`
...
- **Rename/disable dangerous Redis commands** (`FLUSHALL`, `FLUSHDB`,
  ...
  verification.
- **Rotate the Redis password and Qdrant API key** on host migration or
  suspected exposure with `bash scripts/redis-bootstrap.sh --rotate` and
  `bash scripts/qdrant-bootstrap.sh --rotate`, then restart anything
  connected. A re-run without `--rotate` keeps the existing credential, so
  repeating setup never silently disconnects running services.
...
responsibility, and app-layer encryption is off by default), and the Nexus
diagnostics page being readable without the token on loopback — are explicit
operator responsibilities and are tracked above for future versions.
