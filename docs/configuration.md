...
Cooperative pre-boot GPU headroom check. When enabled, the cycle verifies accelerator memory headroom BEFORE any module initializes, so the entity is not OOM-killed mid-init (which Spot would then thrash on). The gate classifies memory as known-discrete (dedicated VRAM), known-unified (system-shared, e.g. Jetson/APU/Apple Silicon), or unknown — and applies `min_free_vram_gb` to the relevant pool; unknown memory always passes with an annotation and never refuses boot. The model backend is a single-resident OpenAI-compatible server (Unsloth Studio / llama.cpp) with no idle-model unload, so reclamation is report-only: the gate measures headroom and reports the server's resident model(s) and other GPU consumers, and never terminates a process — KAINE services (model server / Chatterbox / Speaches) are detected and preserved. If headroom is short it asks the operator to free memory and refuses to boot unless `KAINE_GPU_PREFLIGHT_APPROVED=1` is set.
