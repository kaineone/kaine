...
| 16 | **Mundus** | Embodiment (shipped inactive) | `kaine/modules/mundus/` | Body-agnostic embodiment control plane; routes perception/action to a body through a pluggable adapter (transport-free `stub` reference body ships today; a virtual-world adapter is planned); double-gated (config + operator env var) |
