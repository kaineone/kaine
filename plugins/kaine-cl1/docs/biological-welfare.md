...
KAINE is, unusually, built around *entity welfare* — its license is literally the
...
expensive, so access is being pursued through a grant. Until that access exists,
the **free simulator** (`cl.is_simulator() == True`) is what we build and validate
...
the closed loop against the simulator **first**, so that if and when grant-funded
access to real tissue is secured, we arrive ready — with safe, tested stim
