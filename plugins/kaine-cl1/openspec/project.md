...
- The goal is to run on **real CL1 tissue** (pending grant-funded access). Until
  then we build and validate on the **simulator** (`cl.is_simulator()`); hardware
  runs are a deliberate, reviewed step, never an accidental default.
