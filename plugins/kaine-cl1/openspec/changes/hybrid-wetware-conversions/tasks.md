...
- [ ] 1.1 `WetwareAcousticFrontEnd` behind Audition's front-end salience seam;
      STT/emotion untouched, in `kaine_cl1/backends/audition_frontend.py`.
...
- [ ] 2.1 `WetwareSurprise` sourcing the RSSM surprise scalar from criticality/LZ;
      rollout stays silicon, in `kaine_cl1/backends/phantasia_surprise.py`.
...
- [ ] 3.1 `WetwareActionSelector` closed loop: encode state → stim, decode action
      → intent, in `kaine_cl1/backends/volition.py`.
...
- [ ] 4.1 `WetwareAffect` reading valence/arousal from population dynamics, in
