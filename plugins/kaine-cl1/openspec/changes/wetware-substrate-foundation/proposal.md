...
**free simulator** (`cl.is_simulator()`), the phase before grant-funded access to
real tissue; the same layer drives real cultures once that access exists.
...
- **A substrate broker** — the one genuinely new architectural piece. It leases
...
- Converting any specific module (that is the per-module changes that depend on
  this one).
- Executing on real CL1 hardware *in this change*. Hardware is the project's goal
  but is deferred until grant-funded access exists and its review step is met;
  this change delivers the layer that will drive it, validated on the simulator
  (see `docs/biological-welfare.md`).
