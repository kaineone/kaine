...
- [x] 2.6 `start_beat(blocking=False)` and a background switch from the first `on_cycle_tick`, so a Nous window in flight never stalls KAINE's event loop; `close()` joins a pending switch.
