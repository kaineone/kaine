...
class ScriptedBus:
    ...
    async def read_entries(
        ...
        """Mirrors the real bus contract (H2): (entries, last_scanned) where
        last_scanned is the last entry id scanned, or None when empty. The
        scripted stream has no undecodable entries, so it equals the last
        entry returned."""
