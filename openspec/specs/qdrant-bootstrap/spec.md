# qdrant-bootstrap Specification

## Purpose
Take KAINE's Qdrant vector store from a fresh clone to a running, authenticated
container in one command that is safe to repeat: the API key is created once and
kept on re-runs (rotated only on request), and it is recorded in `compose/.env` and
`config/secrets.toml` without disturbing any other entry.
