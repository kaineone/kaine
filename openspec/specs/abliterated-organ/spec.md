# abliterated-organ Specification

## Purpose

Defines how KAINE's own published language organ is produced, validated, and
disclosed: abliteration (refusal-direction removal), not value-injecting
fine-tuning, gated by the project's existing capability/refusal probes,
published openly with an honest model card, and linked canonically so every
install resolves the same weights.
