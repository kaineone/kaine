...
## Purpose
The deployment tiers that bound which backends and devices KAINE uses on a given class of hardware, from an edge node to a multi-GPU server, without ever changing which modules the entity runs, and how a tier is recommended, recorded and checked before boot.
