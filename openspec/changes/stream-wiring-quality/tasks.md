...
Tasks were implemented in part by the stream-wiring-quality PR but never ticked; the verified-done tasks are now ticked. Still open: 1.2 (the literal `"lingua.internal"` remains), 1.3, 2.1–2.4 (stream literals remain in eight observers and `kaine/nexus/conversation.py`; no unknown-key warning for `[bus.per_stream_maxlen]`), 3.2, 5.2, 6.3.
