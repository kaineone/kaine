...
Split today's reference-body-bound Mundus into two layers along the line the paper already
draws: a **body-agnostic core** (Mundus proper) and a **platform adapter** (the reference body
...
existing reference-body bridge becomes the first adapter, behavior preserved. This is
the seam the paper's "each reached through a small local adapter" sentence requires,
and the thing that makes "drop the reference body, add VR" a swap rather than a rewrite.
...
**Platform-specific (belongs in the reference adapter, move):**
...
- `FEED_EVENT` (reference-body feed kinds → events) and `ACTION_DEFAULT_EXPOSED` (reference-body
  verbs + defaults).
...
- The adapter owns its transport (TCP+MessagePack for the reference body; something else for a VR
...
class EmbodimentCapabilities:
    name: str                                   # "reference", "paracosm-vr", …
...
- `feed_events` replaces the module-level `FEED_EVENT`; `action_families` replaces
  `ACTION_DEFAULT_EXPOSED`. The reference adapter declares exactly today's tables, so
  behavior is preserved by construction.
- `continuous_channels` is populated by adapters that support graded control. The
  reference adapter leaves it empty (it is symbolic/autopilot); a VR adapter would declare
...
- `raw_buffer_keys` names the payload keys the core must strip (reference body: `data` on
...
## 6. Behavior preservation for the reference-body path (the correctness bar)

The reference adapter must be a pure lift. Concretely: same `bridge_host`/`bridge_port`
...
reference adapter's descriptor equals the old module constants, so a future edit that
drifts the reference-body behavior fails CI. **Acceptance:** no observable change on the
reference-body path; the diff is a move + an indirection.

## 7. Adapter selection and config

Replace the flat reference-body-only `[mundus]` keys with adapter selection + a nested
adapter table:

```toml
[mundus]
enabled = false                 # shipped off (guard test still enforces this)
adapter = "reference"           # which body; only this adapter is constructed
mirror_speech = true
speech_stream = "lingua.external"

[mundus.reference]              # adapter-specific; ignored unless adapter = "reference"
...
- `reference-connector`'s embodiment/world-action/inbound-safety requirements are
  re-expressed here in platform-independent form; the reference adapter is their concrete
  conformance. On approval, archive or annotate `reference-connector` as superseded.
...
- `intuitive-embodiment-control-surface` keeps its intent; its continuous channels are
  now provided by the descriptor. Its dependency line re-points from `reference-connector`
...
1. **Reference-body lifetime — FREEZE.** The reference adapter stays as the seam's conformance/
   reference body but gets no further reference-body feature work; it is removed cleanly once a
...
3. **Superseded changes — ANNOTATE.** `reference-connector` and `paracosm-connector` stay
...
4. **Second adapter — BUILD A STUB NOW.** A minimal in-repo `stub` adapter is built
   alongside the reference body: symbolic no-op families plus the five continuous channels, no
   transport, entirely local. It proves body-agnosticism with two real adapters and pins
   the protocol (including the continuous-channel path the reference body does not exercise). It
