...
> `opensim-connector` (its Mundus becomes the OpenSim adapter) and folds
...
- [x] 0.1 Decisions resolved (`design.md` §10): freeze OpenSim, fix channels to
...
- [x] 0.2 Re-read `design.md` §2/§6: the split is a **move + indirection**, not a
      rewrite; the OpenSim path's observable behavior is preserved bit-for-bit.
...
- [x] 2.1 Change `Mundus.__init__` to take an injected `adapter: EmbodimentAdapter`;
      drop `bridge_host`/`bridge_port`/`expose`/OpenSim knowledge from the module.
...
## W3 — OpenSim adapter (`kaine/modules/mundus/adapters/opensim.py`)
...
- [x] 3.3 Declare `capabilities()` with the current OpenSim feed_events, action_families
...
- [x] 3.5.3 Ships unselected and off; a core test drives it to exercise the continuous
      path (clamping, per-channel exposure, locus gate) that OpenSim does not cover.
...
- [x] 4.2 Restructure `config/kaine.toml`: `[mundus].adapter = "opensim"`, move the
      OpenSim keys under `[mundus.opensim]`, keep `enabled = false` shipped.
...
- [x] 5.3 Annotate `opensim-connector` and `paracosm-connector` as `superseded-by:
