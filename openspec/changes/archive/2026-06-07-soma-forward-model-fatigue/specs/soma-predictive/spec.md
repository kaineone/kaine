...
`request_maintenance` MUST set a flag that Hypnos can observe to schedule the
next offline cycle earlier. The cycle SHALL log each advisory action and SHALL
...
- **WHEN** the cycle engine receives a `soma.regulation` event with `action == "request_maintenance"`
- **THEN** a maintenance-requested flag is set that Hypnos reads to schedule an earlier offline cycle
