> ARCHIVED 2026-07-10: implemented via PR #32 — the kaine-side continuous control surface shipped. The 5 remaining tasks were out-of-repo reference-body wire-shim viewer work, mooted by dropping the reference adapter.

# An intuitive, learnable embodiment control surface for Mundus

> **Dependency re-pointed (2026-07-05):** `reference-connector` is superseded by
...
> live — not on `reference-connector` directly. The continuous surface designed here
...
> re-defining them. References to `reference-connector` below read as "the live
> adapter" (the reference body today, VR/paracosm later).

## Why

`reference-connector` designed Mundus's avatar control as a **symbolic verb menu** —
...
- **The symbolic verb menu is NOT the entity's surface.** The stock high-level wire-shim
...
- **The missing producer, built as continuous control.** The `intent.avatar.*`
  producer that `reference-connector` left unbuilt is provided here as a **continuous
...
- **The fork work: continuous control over the wire shim.** The stock viewer's wire shim
  exposes the high-level `LLAgent` verbs and, for raw locomotion, only synthetic `LLWindow`
  ...
  `AGENT_CONTROL_*` flags. Either way this rides the fork `reference-connector` already
  plans. **Verify the fork actually re-enabled wire-shim launching** (the stock viewer
...
- **Refines (does not silently contradict) `reference-connector`:** its
  ...
  continuous surface defined here. When `reference-connector` is implemented/updated,
...
- **Touch points for the future implementer** (design names them; no code here):
  ...
  `tools/mundus-wire/` (continuous-control wire-shim op + feedback), the viewer fork
  (`viewer-fork-notes.md`: the continuous `LLAgent` wire-shim op), the forward-model
...
- **Explicitly NOT touched:** the gestational womb (Change A); the stage machine
  ...
  and inbound-world safety (reused, not modified); the reference-body grid, physics, and
...
- **Relationship to other changes:** entered only after `developmental-maturation-gate`
  flips the stage to `embodied`; depends on `reference-connector`'s Mundus module,
  the wire shim, and viewer fork landing (this change extends them with a continuous
