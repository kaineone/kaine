...
- [x] 1.1 Default `[topos].forward_prediction = true` so the forward-model
...
- [x] 1.2 Replace the absolute `change_alert_threshold` alert test with the same
...
- [x] 1.3 Unit test: a synthetic embedding sequence with a step-change produces an
...
- [x] 2.1 Apply the same self-calibrating alert criterion to the Audition
  general-perception path (`acoustic_change_alert_threshold` → relative), and enable
  its forward-prediction path by default.
- [x] 2.2 Unit test: an acoustic onset raises salience above baseline; steady sound
  stays at baseline.

## 3. Reconcile foveation with the temporal encoder

- [x] 3.1 Retire/correct the stale `Topos.__init__` guard that raises
...
- [x] 3.2 Confirm foveation-on routes the peripheral-gist embedding into the
...
- [x] 4.1 Systems test: a fixture clip with a hard scene cut and an audio onset MUST
...
- [x] 4.2 Operational check: on the reference corpus, log/report the rate of
