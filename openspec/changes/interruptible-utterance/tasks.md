...
- [x] 1.1 Add an `interrupt` flag to the `speak` `Intent` (payload field), default false;
...
- [x] 2.1 Change `_intent_loop` to run each generation as a held `asyncio.Task` instead of
  an inline `await`; catch `CancelledError`, discard the unspoken remainder, write a
  content-free preemption record.
- [x] 2.2 On an interrupt-marked `speak` intent arriving mid-generation, cancel the
  in-flight task and start the new one (redirect).
- [x] 2.3 Tests (spec `interruptible-utterance`): uninterrupted generation completes as
  before; an interrupt aborts + redirects; a partial already emitted is not un-said.

## 3. Interrupt threshold in the report policy

- [x] 3.1 Add `interrupt_threshold` (> report_threshold) to `SelfInitiatedReportPolicy`;
...
- [x] 3.2 Tests: urgent surprise interrupts; below-interrupt reportworthy does not
  preempt; same-content does not interrupt.

## 4. Verification + paper

- [x] 4.1 Suite + `lint-imports`; default (no interrupt) behavior unchanged.
- [x] 4.2 Paper note (review-gated): interruptible/redirectable speech as a property, and
