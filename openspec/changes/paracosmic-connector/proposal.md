...
> `Kosmos` module here) into "one Mundus + per-body adapters." A future Paracosmic
...
> transitional reference body and rebuild Paracosmic around VR-style continuous
> body controls — the eventual
> Paracosmic adapter targets that, not this connector design. Kept for provenance.

## Why

Paracosmic (https://github.com/kaineone/Paracosmic) is a virtual world being built
for embodied cognitive architectures: a Three.js / A-Frame (web/WebXR) world with
procedural terrain, celestial mechanics, weather, fire, lifecycle (mortality
+ memory diamonds + reproduction), and a per-agent bridge process that carries
five sensory feeds (proprio / temporal / intero / visual / audio) plus an action
channel to an external "cognitive agent" process over length-prefixed
MessagePack on raw TCP (default port `7780`).

A KAINE entity should be able to embody into a Paracosmic avatar — receive its
...
without either project being permanently coupled to the other. Paracosmic is
...
Paracosmic cognitive-agent bridge, translates incoming sensory frames into
KAINE bus events, and forwards a new family of `intent.avatar.*` intents
back to Paracosmic as action frames. It also catalogues the gaps on both sides
so the two repos can move forward in lockstep.

## What changes

- **New module:** `kaine/modules/kosmos/` — a `BaseModule` that runs an async
  TCP client against the Paracosmic bridge, decodes the five feed kinds plus
...
- **Eidolon extension:** a `paracosmic_body` field on the self-model populated
...
- **Affected modules:** new `kosmos`; touch points in `eidolon`, `thymos`,
  optionally `topos` (when Paracosmic's real visual feed lands).
- **Affected config:** `config/kaine.toml` (new `[kosmos]` table).
- **Affected docs:** `ARCHITECTURE.md` (new module row), `SETUP.md` (Paracosmic
  connection prereqs), `SECURITY.md` (two-layer gate + per-action opt-ins),
  `FIRST_BOOT.md` (Kosmos defaults off).
- **Cross-project asks** for the Paracosmic repo are listed in
  `paracosmic-counterpart-asks.md` and should be opened as issues there.
