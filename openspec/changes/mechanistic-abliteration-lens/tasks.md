...
- [ ] 0.1 Refusal-eliciting prompt set: reuse/extend the existing
...
- [ ] 0.2 Publish the per-layer readout in the organ model card, or keep it as an
      internal provenance artifact the card references?

## 1. Phase 1 — vendor jlens + smoke on the organ

- [ ] 1.1 Vendor `jlens` under `third_party/jlens/` (pinned to an exact upstream
...
- [ ] 1.2 A thin support module (`kaine`-side, offline) that loads the abliterated
...
- [ ] 1.3 Smoke: fit a small lens on a handful of prompts and decode a known
...
- [ ] 2.1 A refusal-disposition metric: for each layer, the lens-assigned mass on
...
- [ ] 2.2 A research entrypoint `scripts/abliteration_lens.py` that runs the metric
...
- [ ] 3.2 If the readout materially informs §3.5, note the mechanistic result in
