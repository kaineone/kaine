...
- [ ] 2.5 Chronos: size the forward-prediction head from the network's `units` when a network is injected; error if absent while forward prediction is on.
- [ ] 2.6 `make_nous`: skip building `PymdpEngine` when `engine` is injected; keep envelope validation.
...
- [ ] 4.9 Chronos with an injected network of a width other than `cfc_units` runs a forward-prediction tick without error.
