...
`injections` and `make_oscillator` are called once per construction. Spot restarts most modules in place (Chronos and Soma keep their injected object; the plugin is not asked again); modules that hold external resources (Nous among the seamed ones) are rebuilt through `construct_module`, which calls `injections` again, and every heavy rebuild re-creates every module's oscillator through `rewire_module`, which calls `make_oscillator` again for each declared oscillator seam. A plugin that holds exclusive resources per module must accept repeated requests for the same module and hand the resource to the new object, and an injected object must tolerate an in-place shutdown and re-initialization. A declared seam whose returned value is `None` is a `PluginError`, because a module would otherwise build its default model while the log and the manifest record a substitution.
